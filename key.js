...
function strEnc(r) {
    var e="this",a="password",n="is";
    var t, s, o, c, f, m, i = r.length, u = "";
    if (null != e && "" != e && (t = getKeyBytes(e),
    c = t.length),
    null != a && "" != a && (s = getKeyBytes(a),
    f = s.length),
    null != n && "" != n && (o = getKeyBytes(n),
    m = o.length),
    i > 0)
        if (i < 4) {
            var l = strToBt(r);
...
function strDec(r, e, a, n) {
    ...
    var l = parseInt(i / 16)
        , b = 0;
    for (b = 0; b < l; b++) {
        var k, y = r.substring(16 * b + 0, 16 * b + 16), v = hexToBt64(y), g = new Array(64), w = 0;
        for (w = 0; w < 64; w++)
            g[w] = parseInt(v.substring(w, w + 1));
        if (null != e && "" != e && null != a && "" != a && null != n && "" != n) {
            for (A = g,
            B = m - 1; B >= 0; B--)
                A = dec(A, o[B]);
...
function getKeyBytes(r) {
    var e = new Array
        ...
        , s = 0;
    for (s = 0; s < n; s++)
        e[s] = strToBt(r.substring(4 * s + 0, 4 * s + 4));
    return t > 0 && (e[s] = strToBt(r.substring(4 * s + 0, a))),
    e
}
function strToBt(r) {
    var e = r.length
...
function expandPermute(r) {
    var e = new Array(48);
    for (i = 0; i < 8; i++)
        0 == i ? e[6 * i + 0] = r[31] : e[6 * i + 0] = r[4 * i - 1],
        e[6 * i + 1] = r[4 * i + 0],
        e[6 * i + 2] = r[4 * i + 1],
        e[6 * i + 3] = r[4 * i + 2],
        e[6 * i + 4] = r[4 * i + 3],
        7 == i ? e[6 * i + 5] = r[0] : e[6 * i + 5] = r[4 * i + 4];
    return e
}
function xor(r, e) {
...
function sBoxPermute(r) {
    var e = new Array(32)
        ...
        , t = [[15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10], [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5], [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15], [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]]
        ...
        , o = [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15], [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9], [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4], [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]]
        ...
        , f = [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11], [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8], [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6], [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]]
        , i = [[4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1], [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6], [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2], [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]]
